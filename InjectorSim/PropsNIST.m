function [Props] = PropsNIST(T,rho,type)
    %#
    %% INTRODUCTION
    %==========================================================================
    % Purpose:
    % Returns a sturcture containing the thermodynamic properties for saturated
    % nitrous oxide (N2O) or carbon dioxide (CO2) for a given density (kg/m^3) and temperature (K).
    % Valid temperature and pressure ranges are:
    % 183 K ≤ T ≤ 309 K 0 MPa ≤ P ≤ 800 MPa
    %--------------------------------------------------------------------------
    ...
    %==========================================================================
    % CO2 Constants -- idk what these are for N2O, havent changed them -Thomas
    %--------------------------------------------------------------------------
    ...
    %{
    %% Load the N2O properties created by the NIST Webbook
    %==========================================================================
    [labels,T_NIST,y] = readColData('SatProps.txt',25); %doesnt work use
    importdata as follows:
    data = importdata('SatProps.txt')
    y = data.data
    T_NIST = y(:,1);
    ...
    if type == "N2O"
        SatProps = open('SatN2OProps.mat');
    elseif type == "CO2"
        SatProps = open('SatCO2Props.mat');
    else
        return
    end
    T_NIST = SatProps.T_NIST; % Temperature (K)
    P_NIST = SatProps.P_NIST; % Pressure (MPa)
    rho_l_NIST = SatProps.rho_l_NIST; % Density (l, kg/m3)
    rho_v_NIST = SatProps.rho_v_NIST; % Density (v, kg/m3)
    u_l_NIST = SatProps.u_l_NIST; % Internal Energy (l, kJ/kg)
    u_v_NIST = SatProps.u_v_NIST; % Internal Energy (v, kJ/kg)
    h_l_NIST = SatProps.h_l_NIST; % Enthalpy (l, kJ/kg)
    h_v_NIST = SatProps.h_v_NIST; % Enthalpy (v, kJ/kg)
    s_l_NIST = SatProps.s_l_NIST; % Entropy (l, J/g*K)
    s_v_NIST = SatProps.s_v_NIST; % Entropy (v, J/g*K)
    cv_l_NIST = SatProps.cv_l_NIST; % Cv (l, J/g*K)
    cv_v_NIST = SatProps.cv_v_NIST; % Cv (v, J/g*K)
    cp_l_NIST = SatProps.cp_l_NIST; % Cp (l, J/g*K)
    cp_v_NIST = SatProps.cp_v_NIST; % Cp (v, J/g*K)
    c_l_NIST = SatProps.c_l_NIST; % Sound Speed (l, m/s)
    c_v_NIST = SatProps.c_v_NIST; % Sound Speed (v, m/s)
